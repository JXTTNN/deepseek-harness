...
export class WorkspaceManager {
  ...
  async deleteSession(sessionId: SessionId): Promise<RpcResult<{ deleted: boolean; sessionId: SessionId }>> {
    const { result } = await this.api.workspace.deleteSession({ sessionId })
    if (result.ok) this.installArchived([...this.archivedSessionIds, sessionId])
    return result
  }
