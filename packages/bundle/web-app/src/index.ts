...
export interface Config {
  ...
  trustedHosts: string[]
  /**
   * Absolute path of the runtime log file. One structured JSON line is
   * appended per log record; absent or empty disables file logging.
   */
  logFile?: string
  /** Minimum severity recorded to {@link Config.logFile}. */
...
export const Config: z<Config> = z.object({
  ...
  logLevel: z.union(['error', 'info', 'warn', 'debug'] as const).default('debug'),
})
...
/** Severity name to numeric threshold for the file logger. */
const LOG_LEVELS: Readonly<Record<NonNullable<Config['logLevel']>, number>> = {
  error: 0,
  info: 1,
  warn: 2,
  debug: 3,
...
/**
 * Register a structured runtime log file on the root context. Every log record
 * the runtime emits at or above `level` is appended as one JSON line, so a run
 * can be inspected after the fact.
 * @param ctx - plugin context.
 * @param file - absolute log file path (its directory is created).
 * @param level - minimum severity to record.
 * @returns a disposer that closes the stream.
 */
function registerFileLogger(ctx: Context, file: string, level: NonNullable<Config['logLevel']>): () => void {
  mkdirSync(dirname(file), { recursive: true })
  const stream = createWriteStream(file, { flags: 'a' })
  const line = (text: string): void => { stream.write(`${text}\n`) }
  line(`=== dsh log start ${new Date().toISOString()} pid=${process.pid} level=${level} file=${file} ===`)
  // `Logger.format` reads only `colors`/`maxLength`/`formatters`, but the
  // `Exporter` interface also requires `export`; the no-op satisfies the type.
  const formatter: Exporter = { colors: false, maxLength: 4096, export: () => {} }
  ctx.logger.exporter({
    colors: false,
    maxLength: 4096,
    levels: { default: LOG_LEVELS[level] },
    export: (message) => {
      line(JSON.stringify({
        ts: new Date(message.ts).toISOString(),
        level: message.type,
        name: message.name,
        msg: Logger.format(formatter, message),
      }))
    },
  })
  return () => { stream.end() }
 ...
 * variable, the URL line, and (when configured) the runtime log file.
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (logFile !== '') {
    ctx.effect(() => registerFileLogger(ctx, logFile, config.logLevel ?? 'debug'), 'web-app: runtime log file')
  }
