...
import { closeSync, existsSync, fsyncSync, mkdirSync, openSync, readFileSync, readdirSync, rmSync, statSync, unlinkSync, writeFileSync } from 'node:fs'
...
// Shared constants, types, and helpers (extracted to shared.ts)
import {
  TEAM_DIR, MAX_MESSAGE_BYTES, MAX_THINK_LOG_LINES,
  MAX_INBOX_MESSAGES, DEDUP_WINDOW_MS, SERVER_PORT, MAX_APPEND_FILE_BYTES,
  type TeamMessage, type TeamTask, type TeamMemoryEntry,
  type TeamBroadcast, type TeamReview, type TeamSent, type TeamBarrier,
  type StructuredEvidence, type TeamReport,
  teamCwd, isSafeTeamId, assertSafeTeamId, teamPath,
  withFileLock, lockedAppend, lockedUpdate,
  peerIds, deliverMessage, triggerSession, notifyPeer,
  readJsonl, readJsonlStrict, readTailJsonl, writeJsonl,
  writePresence, readAllPresence,
  rankMemoryEntries,
  writeSetsOverlap,
} from './shared'

// Deep optimization: durable approvals, governance, execution-receipt, recovery, plan-replay
import { DurableApprovalLedger, createApprovalRequest } from './approval/durable'
import { evaluateGovernance, buildGovernanceTaskSpecs } from './governance'
import type { GovernanceDeclaration } from './governance'
import { resolveRecoveryOptions, validatePlanPatch, planPatchSignature } from './recovery'
import { createPlanArtifact, validatePlanArtifact, savePlanArtifact, loadPlanArtifact, listPlanArtifacts, deletePlanArtifact } from './plan-replay'

// GitHub skill search and injection for team_spawn
import {
  searchGitHubSkills, searchGitHubSkillFiles,
  loadGitHubSkillsFromStrings,
  type GitHubSkillSearchResult,
} from './github-skills'

// Election, role, auto-assign, and agent spec helpers (extracted to election.ts)
import {
  ELECTION_LEASE_MS,
  type ElectionRecord,
  readElection, writeElection, electLeader,
  type RoleDefinition, type RoleAssignments,
  readRole, writeRole, readRoleAssignments, writeRoleAssignments,
  readAutoAssignState, writeAutoAssignState,
  type AgentSpec,
  readAgentSpec, writeAgentSpec,
} from './election'

// Spawn helpers (extracted to spawn.ts)
import {
  DEFAULT_SPAWN_PROVIDER, DEFAULT_SPAWN_MAX_DEPTH,

  renderSkillForPrompt, writeSpawnRecord, listSpawnRecords,
} from './spawn'

// Workflow DAG helpers (extracted to workflow.ts)
import {
  type WorkflowNode, type WorkflowEdge, type WorkflowState,
  readWorkflow, writeWorkflow, isNodeReady,
} from './workflow'

// Audit helpers (extracted to audit.ts)
import {
  type AuditEvent,
  readAllAuditEvents,
} from './audit'

// Permission helpers (extracted to permission.ts)
import {
  PERMISSION_LAYERS,
  readPermissionRules, writePermissionRules,
} from './permission'

// Contract helpers (extracted to contract.ts)
import {
  createContract, readContract, listContracts,
  updateContractStatus, verifyContract,
  type ContractStatus,
} from './contract'
...
export function apply(ctx: Context): void {
  ...
  if (subagentsService !== undefined) {
    ...
    ctx.tools.register(_defineToolAny({
      ...
      parameters: {
        ...
        githubSkills: { type: 'array', items: { type: 'string' }, description: 'GitHub skill references to search and inject (e.g. ["owner/repo/SKILL.md", "owner/repo@branch/path"]). Ephemeral: downloaded on-demand, injected into prompt, not persisted. Max 5 skills.' },
        toolFilter: {
          type: 'object',
          additionalProperties: false,
          properties: {
      ...
      async execute(args: any, exec: any) {
        ...
        // "live": load GitHub skills on demand - ephemeral, no caching, no persistence
        const githubSkillRefs: string[] = Array.isArray(args.githubSkills) ? args.githubSkills : []
        const ghResult = await loadGitHubSkillsFromStrings(githubSkillRefs, { signal: exec.signal })
        const fullSkillPrefix = skillPrefix + ghResult.prefix
        const allSkillsFound = [...skillsFound, ...ghResult.found]
        const allSkillsMissing = [...skillsMissing, ...ghResult.missing]

        // Build the prompt: skill instructions first, then the user's task prompt
        const fullPromptText = fullSkillPrefix + args.prompt
        ...
        if (mode === 'continuable') {
          ...
          return { kind: 'continuable' as const, subagentId: started.childId, messageId: started.messageId, allSkillsFound, skillsMissing }
        }
    ...
    ctx.tools.register(_defineToolAny({
      ...
      async execute(args: any, exec: any) {
        ...
        const skillNames: string[] = Array.isArray(args.skills) ? args.skills : []
        const cwd = teamCwd(agent)
        const { prefix: skillPrefix, found: allSkillsFound, missing: skillsMissing } = await loadSkillsForSpawn(skillNames, cwd, exec.signal)
        ...
        return { messageId, allSkillsFound, skillsMissing }
      },
      presentCall: (args: any) => ({
        card: 'generic' as const,
    ...
    // -- team_github_skill_search: search GitHub for skills --

    ctx.tools.register(_defineToolAny({
      name: 'team_github_skill_search',
      description:
        'Search GitHub for agent skill repositories and SKILL.md files. '
        + 'Returns matching skills with owner, repo, path, stars, and URL. '
        + 'Use the full reference (owner/repo/path) with team_spawn\'s githubSkills '
        + 'parameter to inject the skill into a subagent. Skills are ephemeral: '
        + 'downloaded on-demand, injected into the prompt, and not persisted. '
        + 'Requires GITHUB_TOKEN or GH_TOKEN environment variable for API access.',
      parameters: {
        query: { type: 'string', required: true, description: 'Search term (matched against repo name, description, and SKILL.md content).' },
        language: { type: 'string', description: 'Optional language filter (e.g. "typescript", "python").' },
        searchMode: { type: 'string', enum: ['repos', 'files', 'both'], description: 'Search mode: repos (search skill repos), files (search SKILL.md files), both (default).' },
        limit: { type: 'number', description: 'Max results per search mode (default 10, max 30).' },
      },
      output: {
        schema: {
          type: 'object',
            ...
            skills: {
              ...
              items: {
                ...
                properties: {
                  ...
                  ref: { type: 'string', description: 'Full reference string for use with team_spawn githubSkills parameter (owner/repo/path).' },
                },
              },
            },
            totalFound: { type: 'number', required: true },
          },
        },
        render: (_args: any, value: any) => [{
          type: 'text',
          text: value.skills.length === 0
            ? 'No GitHub skills found. Try a different query or set GITHUB_TOKEN.'
            : `${value.totalFound} GitHub skill(s) found:\n` + value.skills.map((s: any) =>
              `  ${s.ref} (${s.stars} stars): ${s.description.substring(0, 100)}`).join('\n'),
        }],
      },
      isConcurrencySafe: () => true,
      async execute(args: any, exec: any) {
        const agent = exec.agent
        if (!agent) throw new Error('team_github_skill_search: no agent context')

        const mode = args.searchMode ?? 'both'
        const limit = args.limit ?? 10
        const results: GitHubSkillSearchResult[] = []

        if (mode === 'repos' || mode === 'both') {
          try {
            const repoResults = await searchGitHubSkills(args.query, {
              ...(args.language !== undefined ? { language: args.language } : {}),
              limit,
              signal: exec.signal,
            })
            results.push(...repoResults)
          } catch { /* best-effort: repo search may fail without token */ }
        }

        if (mode === 'files' || mode === 'both') {
          try {
            const fileResults = await searchGitHubSkillFiles(args.query, {
              limit,
              signal: exec.signal,
            })
            results.push(...fileResults)
          } catch { /* best-effort: code search may fail without token */ }
        }

        // Deduplicate by owner/repo/path
        const seen = new Set<string>()
        const deduped = results.filter((s) => {
          const key = `${s.owner}/${s.repo}/${s.path}`
        ...
        return {
          skills: deduped.map((s) => ({
            ...
            ...(s.topics.length > 0 ? { topics: s.topics } : {}),
            ref: `${s.owner}/${s.repo}/${s.path}`,
          })),
      ...
      presentCall: (args: any) => ({
        card: 'generic' as const,
        title: `GitHub skill search: ${args.query}`,
        kind: 'list' as const,
      }),
    }))
  ...
  // ==========================================================================
  // -- F12: Durable Approval Gates -------------------------------------------
  // Content-bound approval requests with SHA-256 hash binding and atomic
  // first-decision-wins semantics. Four boundaries: plan, task_round,
  // task_dispatch, tool_call. Stored in .team/approvals/<requestId>.json.
  // ==========================================================================

  ctx.tools.register(_defineToolAny({
    name: 'team_approval',
    description:
      'F12: Durable approval gates with SHA-256 content hash binding. '
      + 'Actions: create (create an approval request at a boundary), decide (approve/reject a request), '
      + 'get (read an approval record), list (list all approval records). '
      + 'Four boundaries: plan, task_round, task_dispatch, tool_call. '
      + 'Decisions are atomic: first decision wins, concurrent or repeated decisions fail. '
      + 'Records stored in .team/approvals/<requestId>.json.',
    parameters: {
      action: { type: 'string', required: true, enum: ['create', 'decide', 'get', 'list'], description: 'Which approval operation to run.' },
      scope: { type: 'string', enum: ['plan', 'task_round', 'task_dispatch', 'tool_call'], description: 'Approval boundary (create action).' },
      boundary: { type: 'string', description: 'Boundary identifier (create action).' },
      runId: { type: 'string', description: 'Run identifier (create action).' },
      content: { type: 'json', description: 'Approval request content (create action).' },
      reason: { type: 'string', description: 'Human-readable reason for the request (create action).' },
      requestId: { type: 'string', description: 'Approval request id (decide/get actions).' },
      requestHash: { type: 'string', description: 'SHA-256 hash of the reviewed content (decide action).' },
      decision: { type: 'string', enum: ['approve', 'reject'], description: 'Decision (decide action).' },
      reviewerId: { type: 'string', description: 'Reviewer session id (decide action).' },
      reviewerDisplayName: { type: 'string', description: 'Reviewer display name (decide action).' },
    },
    output: {
      schema: {
        type: 'object',
      ...
      render: (args: any, value: any) => [{
        ...
        text: args.action === 'create'
          ? `approval request created: ${value.requestId}`
          : args.action === 'decide'
            ? `approval ${value.decision}: ${value.requestId}`
            : args.action === 'get'
              ? `approval record: ${value.record?.request?.id ?? 'not found'}`
              : `${value.records?.length ?? 0} approval record(s)`,
      }],
    },
    async execute(args: any, exec: any) {
      const agent = exec.agent
      if (!agent) throw new Error('team_approval: no agent context')
      const cwd = teamCwd(agent)
      const teamDir = join(cwd, TEAM_DIR)
      const ledger = new DurableApprovalLedger(teamDir)

      if (args.action === 'create') {
        if (!args.scope || !args.boundary || !args.runId || !args.content) {
          throw new Error('team_approval create: scope, boundary, runId, and content are required')
        }
        ...
        const record = await ledger.ensureRequest(request)
        return { action: 'create', requestId: request.id, requestHash: request.requestHash, record }
      }

      if (args.action === 'decide') {
        if (!args.requestId || !args.requestHash || !args.decision || !args.reviewerId) {
          throw new Error('team_approval decide: requestId, requestHash, decision, and reviewerId are required')
        }
        const decisionRecord = await ledger.decide({
          requestId: args.requestId,
          requestHash: args.requestHash,
          decision: args.decision,
          reviewer: {
            id: args.reviewerId,
            ...(args.reviewerDisplayName !== undefined ? { displayName: args.reviewerDisplayName } : {}),
          },
        })
        return { action: 'decide', requestId: decisionRecord.requestId, decision: decisionRecord.decision }
      }

      if (args.action === 'get') {
        if (!args.requestId) throw new Error('team_approval get: requestId is required')
        const record = await ledger.get(args.requestId)
      ...
      throw new Error(`team_approval: unknown action ${args.action}`)
    },
    presentCall: (args: any) => ({
      card: 'generic' as const,
      title: `Approval ${args.action}: ${args.requestId ?? args.scope ?? ''}`,
      kind: args.action === 'create' ? 'create' : args.action === 'decide' ? 'other' : 'read',
    }),
  }))

  // ==========================================================================
  // -- F13: Governance Floor -------------------------------------------------
  // Declarative governance: required roles, required order, independent
  // review verification. Evaluate governance compliance against execution
  // receipts, or build governance task specs for structured runs.
  // ==========================================================================

  ctx.tools.register(_defineToolAny({
    name: 'team_governance',
    description:
      'F13: Governance floor - declarative governance with role and order validation. '
      + 'Actions: evaluate (check governance compliance against an execution receipt), '
      + 'build (construct governance task specs from a declaration). '
      + 'Governance intent: "required" (must satisfy) or "preferred" (should satisfy). '
      + 'Required roles: all must execute. Required order: execution must follow declared sequence '
      + 'with dependency paths. Independent review: with 2+ roles, dependency edges must exist.',
    parameters: {
      action: { type: 'string', required: true, enum: ['evaluate', 'build'], description: 'Which governance operation to run.' },
      governanceIntent: { type: 'string', enum: ['required', 'preferred'], description: 'Governance intent.' },
      requiredRoles: { type: 'array', items: { type: 'string' }, description: 'Roles that must participate.' },
      requiredOrder: { type: 'array', items: { type: 'string' }, description: 'Required execution order of roles.' },
      receipt: { type: 'json', description: 'Execution receipt to evaluate against (evaluate action).' },
      goal: { type: 'string', description: 'Goal description (build action).' },
      agentNames: { type: 'array', items: { type: 'string' }, description: 'Available agent names (build action).' },
    },
    output: {
      schema: {
        type: 'object',
        ...
        properties: {
          ...
          taskSpecs: { type: 'array', items: { type: 'json' } },
        },
      },
      render: (args: any, value: any) => [{
        type: 'text',
        text: args.action === 'evaluate'
          ? `governance: ${value.conclusion}`
          : `${value.taskSpecs?.length ?? 0} governance task spec(s)`,
      }],
    },
    async execute(args: any, exec: any) {
      const agent = exec.agent
      if (!agent) throw new Error('team_governance: no agent context')

      if (args.action === 'evaluate') {
        const declaration: GovernanceDeclaration = {
          ...(args.governanceIntent !== undefined ? { governanceIntent: args.governanceIntent } : {}),
          ...(args.requiredRoles !== undefined ? { requiredRoles: args.requiredRoles } : {}),
          ...(args.requiredOrder !== undefined ? { requiredOrder: args.requiredOrder } : {}),
        }
        ...
        const conclusion = evaluateGovernance(declaration, receipt)
        return { action: 'evaluate', conclusion }
      }

      if (args.action === 'build') {
        if (!args.goal) throw new Error('team_governance build: goal is required')
        if (!args.agentNames || !Array.isArray(args.agentNames)) {
          throw new Error('team_governance build: agentNames is required')
        }
        const declaration: GovernanceDeclaration = {
          ...(args.governanceIntent !== undefined ? { governanceIntent: args.governanceIntent } : {}),
          ...(args.requiredRoles !== undefined ? { requiredRoles: args.requiredRoles } : {}),
        ...
        const taskSpecs = buildGovernanceTaskSpecs(args.goal, args.agentNames, declaration)
        return { action: 'build', taskSpecs }
      }

      throw new Error(`team_governance: unknown action ${args.action}`)
    },
    presentCall: (args: any) => ({
      card: 'generic' as const,
      title: `Governance ${args.action}`,
      kind: 'other' as const,
    }),
  }))

  // ==========================================================================
  // -- F14: Adaptive Recovery ------------------------------------------------
  // Opt-in runtime plan repair: when a task fails or is rejected, a Replanner
  // can propose a PlanPatch to add/retarget/supersede pending tasks.
  // Patches are validated, gated by onPlanPatch, and applied atomically.
  // ==========================================================================

  ctx.tools.register(_defineToolAny({
    name: 'team_recover',
    description:
      'F14: Adaptive recovery - runtime plan repair for failed or rejected tasks. '
      + 'Actions: resolve (resolve recovery options from configuration), '
      + 'validate (validate a plan patch against existing tasks and agents), '
      + 'patch (apply a plan patch: addTasks, retargetPending, supersedePending). '
      + 'Limits: maxPlanRevisions (default 3), maxAddedTasks (default 20). '
      + 'A patch adds replacement tasks rather than rewriting history.',
    parameters: {
      action: { type: 'string', required: true, enum: ['resolve', 'validate', 'patch'], description: 'Which recovery operation to run.' },
      configured: { type: 'json', description: 'Configured recovery options (resolve action).' },
      perRun: { type: 'json', description: 'Per-run recovery options (resolve action).' },
      patch: { type: 'json', description: 'Plan patch to validate or apply (validate/patch actions).' },
      existingTasks: { type: 'array', items: { type: 'json' }, description: 'Existing task snapshots (validate action).' },
      agentNames: { type: 'array', items: { type: 'string' }, description: 'Available agent names (validate action).' },
    },
    output: {
      schema: {
        type: 'object',
      ...
      render: (args: any, value: any) => [{
        type: 'text',
        text: args.action === 'resolve'
          ? `recovery mode: ${value.resolved?.mode ?? 'unknown'}`
          : args.action === 'validate'
            ? value.valid ? 'patch VALID' : `patch INVALID: ${value.error}`
            : `patch applied: ${value.signature}`,
      }],
    },
    async execute(args: any, exec: any) {
      const agent = exec.agent
      if (!agent) throw new Error('team_recover: no agent context')

      if (args.action === 'resolve') {
        const resolved = resolveRecoveryOptions(args.configured, args.perRun)
        return { action: 'resolve', resolved }
      }

      if (args.action === 'validate') {
        if (!args.patch) throw new Error('team_recover validate: patch is required')
        try {
          validatePlanPatch(args.patch, args.existingTasks ?? [], args.agentNames ?? [])
          return { action: 'validate', valid: true }
        } catch (error: unknown) {
          return { action: 'validate', valid: false, error: String(error) }
        }
      }

      if (args.action === 'patch') {
        if (!args.patch) throw new Error('team_recover patch: patch is required')
        const signature = planPatchSignature(args.patch)
        return { action: 'patch', signature }
      }

      throw new Error(`team_recover: unknown action ${args.action}`)
    },
    presentCall: (args: any) => ({
      card: 'generic' as const,
      title: `Recovery ${args.action}`,
      kind: 'other' as const,
    }),
  }))

  // ==========================================================================
  // -- F15: Plan Preview & Replay --------------------------------------------
  // Freeze a plan into a serializable artifact, edit it, and replay it
  // without calling the coordinator. Plan artifacts are stored in
  // .team/plans/<name>.json.
  // ==========================================================================

  ctx.tools.register(_defineToolAny({
    name: 'team_plan',
    description:
      'F15: Plan preview and replay - freeze a plan into a serializable artifact, '
      + 'edit it, and replay it without calling the coordinator. '
      + 'Actions: create (create a plan artifact from a plan-only preview), '
      + 'validate (validate a plan artifact), '
      + 'save (persist a plan artifact to .team/plans/<name>.json), '
      + 'load (load a plan artifact from file), '
      + 'list (list all saved plan artifacts), '
      + 'delete (remove a plan artifact). '
      + 'Plan artifacts are plain JSON: diff, commit, and hand to another process.',
    parameters: {
      action: { type: 'string', required: true, enum: ['create', 'validate', 'save', 'load', 'list', 'delete'], description: 'Which plan operation to run.' },
      preview: { type: 'json', description: 'Plan-only preview result (create action).' },
      plan: { type: 'json', description: 'Plan artifact (validate/save actions).' },
      name: { type: 'string', description: 'Plan artifact name (save/load/list/delete actions).' },
    },
    output: {
      schema: {
        type: 'object',
      ...
      render: (args: any, value: any) => [{
        type: 'text',
        text: args.action === 'create'
          ? `plan artifact created with ${value.artifact?.tasks?.length ?? 0} task(s)`
          : args.action === 'validate'
            ? value.valid ? 'plan VALID' : `plan INVALID: ${value.error}`
            : args.action === 'save'
              ? `plan saved: ${value.path}`
              : args.action === 'load'
                ? value.artifact ? `plan loaded: ${value.artifact.tasks?.length ?? 0} task(s)` : 'plan not found'
                : args.action === 'list'
                  ? `${value.names?.length ?? 0} plan(s)`
                  : `plan deleted: ${args.name}`,
      }],
    },
    async execute(args: any, exec: any) {
      const agent = exec.agent
      if (!agent) throw new Error('team_plan: no agent context')
      const cwd = teamCwd(agent)
      const teamDir = join(cwd, TEAM_DIR)

      if (args.action === 'create') {
        if (!args.preview) throw new Error('team_plan create: preview is required')
        const artifact = createPlanArtifact(args.preview)
        return { action: 'create', artifact }
      }

      if (args.action === 'validate') {
        if (!args.plan) throw new Error('team_plan validate: plan is required')
        try {
          validatePlanArtifact(args.plan)
          return { action: 'validate', valid: true }
        } catch (error: unknown) {
          return { action: 'validate', valid: false, error: String(error) }
        }
      ...
      if (args.action === 'save') {
        if (!args.plan) throw new Error('team_plan save: plan is required')
        if (!args.name) throw new Error('team_plan save: name is required')
        const path = savePlanArtifact(args.plan, teamDir, args.name)
        return { action: 'save', saved: true, path }
      }

      if (args.action === 'load') {
        if (!args.name) throw new Error('team_plan load: name is required')
        const artifact = loadPlanArtifact(teamDir, args.name)
        return { action: 'load', artifact }
      }

      if (args.action === 'list') {
        const names = listPlanArtifacts(teamDir)
        return { action: 'list', names }
      }

      if (args.action === 'delete') {
        if (!args.name) throw new Error('team_plan delete: name is required')
        const deleted = deletePlanArtifact(teamDir, args.name)
        return { action: 'delete', deleted }
      }

      throw new Error(`team_plan: unknown action ${args.action}`)
    },
    presentCall: (args: any) => ({
      card: 'generic' as const,
      title: `Plan ${args.action}: ${args.name ?? ''}`,
      kind: args.action === 'save' ? 'create' : args.action === 'delete' ? 'delete' : 'read',
    }),
  }))

  // ==========================================================================
  // -- Task Contract: define and verify contracts between agents ------------
  // ==========================================================================

  ctx.tools.register(_defineToolAny({
    name: 'team_contract',
    description:
      'Task contract definition and verification. A contract specifies what a producer'
      + ' agent promises to deliver and what a consumer agent expects.'
      + ' Actions: create (propose a contract), accept (consumer accepts),'
      + ' deliver (producer delivers output), verify (check output against schema),'
      + ' list (list contracts), read (read a contract), cancel (cancel a contract).'
      + ' Contracts stored in .team/contracts/.',
    parameters: {
      action: { type: 'string', required: true, enum: ['create', 'accept', 'deliver', 'verify', 'list', 'read', 'cancel'], description: 'Contract operation.' },
      id: { type: 'string', description: 'Contract id (read/accept/deliver/verify/cancel).' },
      consumer: { type: 'string', description: 'Consumer session id (create).' },
      description: { type: 'string', description: 'What the contract is about (create).' },
      outputSchema: { type: 'json', description: 'Expected output shape as JSON schema fragment (create).' },
      inputSchema: { type: 'json', description: 'Expected input shape (create, optional).' },
      deadline: { type: 'string', description: 'ISO deadline (create, optional).' },
      deliverable: { type: 'json', description: 'Delivered payload (deliver).' },
      status: { type: 'string', enum: ['proposed', 'accepted', 'delivered', 'verified', 'breached', 'cancelled'], description: 'Filter by status (list).' },
      participant: { type: 'string', description: 'Filter by participant session id (list).' },
    },
    output: {
      schema: {
        type: 'object',
        ...
        properties: {
          action: { type: 'string', required: true },
          contract: { type: 'json' },
          contracts: { type: 'array', items: { type: 'json' } },
          valid: { type: 'boolean' },
      ...
      render: (args: any, value: any) => [{
        type: 'text',
        text: args.action === 'create'
          ? 'contract ' + value.contract?.id + ' proposed'
          : args.action === 'accept'
            ? 'contract ' + args.id + ' accepted'
            : args.action === 'deliver'
              ? 'contract ' + args.id + ' delivered'
              : args.action === 'verify'
                ? value.valid ? 'contract ' + args.id + ' VALID' : 'contract ' + args.id + ' INVALID: missing ' + (value.missing ?? []).join(', ')
                : args.action === 'list'
                  ? (value.contracts?.length ?? 0) + ' contract(s)'
                  : args.action === 'read'
                    ? value.contract ? 'contract ' + value.contract.id + ': ' + value.contract.status : 'contract not found'
                    : 'contract ' + args.id + ' cancelled',
      }],
    },
    async execute(args: any, exec: any) {
      const agent = exec.agent
      if (!agent) throw new Error('team_contract: no agent context')

      if (args.action === 'create') {
        if (!args.consumer) throw new Error('team_contract create: consumer required')
        if (!args.description) throw new Error('team_contract create: description required')
        if (!args.outputSchema) throw new Error('team_contract create: outputSchema required')
        ...
        return { action: 'create', contract }
      }

      if (args.action === 'accept') {
        if (!args.id) throw new Error('team_contract accept: id required')
        const contract = updateContractStatus(agent, args.id, 'accepted')
        if (!contract) throw new Error('team_contract: contract not found: ' + args.id)
        return { action: 'accept', contract }
      }

      if (args.action === 'deliver') {
        if (!args.id) throw new Error('team_contract deliver: id required')
        if (!args.deliverable) throw new Error('team_contract deliver: deliverable required')
        const contract = updateContractStatus(agent, args.id, 'delivered', { deliverable: args.deliverable })
        if (!contract) throw new Error('team_contract: contract not found: ' + args.id)
        return { action: 'deliver', contract }
      }

      if (args.action === 'verify') {
        if (!args.id) throw new Error('team_contract verify: id required')
        const result = verifyContract(agent, args.id)
        if (result.valid) {
          updateContractStatus(agent, args.id, 'verified')
        }
        return { action: 'verify', valid: result.valid, missing: result.missing, contract: result.contract }
      }

      if (args.action === 'list') {
        const filter: { status?: ContractStatus; participant?: string } = {}
        if (args.status) filter.status = args.status
        if (args.participant) filter.participant = args.participant
        const contracts = listContracts(agent, Object.keys(filter).length > 0 ? filter : undefined)
        return { action: 'list', contracts }
      }

      if (args.action === 'read') {
        if (!args.id) throw new Error('team_contract read: id required')
        const contract = readContract(agent, args.id)
        return { action: 'read', contract }
      }

      if (args.action === 'cancel') {
        if (!args.id) throw new Error('team_contract cancel: id required')
        const contract = updateContractStatus(agent, args.id, 'cancelled')
        if (!contract) throw new Error('team_contract: contract not found: ' + args.id)
        return { action: 'cancel', contract }
      }

      throw new Error('team_contract: unknown action ' + args.action)
    },
    presentCall: (args: any) => ({
      card: 'generic' as const,
