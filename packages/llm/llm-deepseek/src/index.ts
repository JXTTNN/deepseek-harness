...
export interface Config {
  ...
  /** Provider-owned model-request retry policy; omission defaults to unbounded `always` recovery. */
...
export function resolveAdapterOptions(config: Config, environment?: LaunchEnvironmentSnapshot): ResolvedDeepSeekOptions {
  ...
  return {
    ...
    streamIdleTimeoutMs,
    // Default to unbounded ("always") recovery so the DeepSeek provider never
    // gives up on a transient model-request failure; an explicit `retryPolicy`
    // (including `mode: 'normal'`) still overrides this.
    retryPolicy: resolveRetryPolicy(config.retryPolicy ?? { mode: 'always' }, 'llm-deepseek: retryPolicy'),
  }
}
