...
- **always mode 会重试其他永久性失败**：除致命的 `HTTP_401`／`HTTP_402`／`HTTP_403` code 外，身份验证、配额、无效请求、协议和无法恢复的上下文错误都会继续重试，直至成功、取消或 dispose；部署负责提供方特定的成本与延迟控制。
