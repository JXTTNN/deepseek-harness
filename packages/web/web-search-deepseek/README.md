...
It reuses the `DEEPSEEK_API_KEY` credential reference (no new secret) and optionally follows `$DEEPSEEK_BASE_URL` from the LLM chat-completions configuration. When `$DEEPSEEK_BASE_URL` is set, the search seam uses the same endpoint base as the LLM adapter. A mounted credentials service is authoritative; without one, the provider falls back to `$DEEPSEEK_SEARCH_BASE_URL` or the Anthropic-compatible default. The reference is resolved for each search, so a key or endpoint stored or rotated by the Web Models page reaches the next call without a restart.
...
| `baseURL` | `https://api.deepseek.com/anthropic/v1` | Anthropic-compatible endpoint base; `/messages` is appended. Falls back to `$DEEPSEEK_BASE_URL` (LLM configuration) or `$DEEPSEEK_SEARCH_BASE_URL` from any environment layer; an unparseable value makes the provider unavailable. |
| `model` | `deepseek-v4-flash` | Anthropic-format model name. |
| `apiVersion` | `2023-06-01` | `anthropic-version` header value. |
| `maxTokens` | `32768` | Positive-integer upper bound on generated tokens for the Messages request. |
| `maxUses` | `10` | Positive-integer maximum `web_search` server-tool uses per request. |
