  ...
  return {
    ...
    workspace: {
      ...
      async deleteSession(request) {
        const { sessionId } = request.payload
        // Defense-in-depth: the id is interpolated into a filesystem path below,
        // so reject separators / traversal / NUL before touching the disk.
        if (/[\\/]/.test(sessionId) || sessionId.includes('\0') || sessionId === '.' || sessionId === '..') {
          return err(request, { code: 'session-not-found', message: `invalid session id "${sessionId}"`, details: { sessionId } })
        }
        try {
          ...
          //    live, so sweep every workspace path the registry knows in addition
          //    to the live agent's cwd, instead of relying on a live agent only.
          const teamRoots = new Set<string>()
          const liveCwd = ctx.agents.get(sessionId)?.session?.header?.cwd
          if (liveCwd) teamRoots.add(liveCwd)
          for (const ws of ctx.workspaceRegistry.list()) teamRoots.add(ws.path)
          for (const root of teamRoots) {
            try {
              const presenceFile = join(root, '.team', 'presence', `${sessionId}.json`)
              if (existsSync(presenceFile)) unlinkSync(presenceFile)
              const inboxFile = join(root, '.team', 'inbox', `${sessionId}.jsonl`)
              if (existsSync(inboxFile)) unlinkSync(inboxFile)
            } catch { /* best-effort */ }
          }
          return ok(request, { deleted: true, sessionId })
        } catch (error: unknown) {
          if (!(error instanceof WorkspaceUnknownSessionError)) throw error
          return err(request, { code: 'session-not-found', message: error.message, details: { sessionId } })
        }
