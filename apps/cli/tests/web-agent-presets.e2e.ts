...
describe('the shipped Web composition', () => {
  ...
  it('composes the full agent from `standard`', async () => {
    ...
    try {
      ...
      expect(toolNames(ctx, handle.agent).filter(name => name !== 'glob' && name !== 'grep')).toEqual([
        ...
        'create_goal', 'edit', 'exit_plan_mode', 'get_goal',
        'github_file_read', 'github_issue_create', 'github_issue_list', 'github_pr_create', 'github_search_code',
        'interrupt_agent', 'job_kill', 'job_list', 'job_output', 'list_agents',
        // The preset mounts both shell tools; a `disabled:` gate keeps exactly one per platform.
        ...(process.platform === 'win32' ? ['pwsh'] : []),
        'ralph', 'read', 'read_image', 'send_message', 'session_delete', 'skill',
        'subagent', 'subagent_fork',
        'team_barrier', 'team_broadcast', 'team_collect', 'team_inbox', 'team_list', 'team_memory',
        'team_report', 'team_review', 'team_review_collect', 'team_send', 'team_status', 'team_task',
        'team_think_read', 'team_wrap', 'think',
  ...
  it('composes the DeepSeek team-lead from `team`', async () => {
    const handle = await ctx.agents.create({
      sessionId: SessionId('preset-team'),
      setup: agentCtx => ctx.agentPresets.mount(agentCtx, 'team').then(() => undefined),
    })
    try {
      // Team Mode = the standard agent plus a team-lead persona: the whole
      // collaboration surface (subagents, workflows, supervision) and the
      // long-task goal memory tools all reach the model.
      ...
      expect(tools).toEqual(expect.arrayContaining([
        ...
        'workflow', 'ralph', 'todo_write',
        'team_send', 'team_inbox', 'team_list', 'think',
      ...
      expect(persona?.text).toContain('DeepSeek team-lead agent')
  ...
  it('composes the cordis agent with its own toolset', async () => {
    ...
    try {
      ...
      // The shell pick is platform-gated in the preset: bash everywhere, pwsh on Windows (which replaces bash there).
...
describe('authoring a preset on the shipped composition', () => {
  ...
  it('copies a shipped preset a session then really composes from', async () => {
    ...
    // Node cannot model POSIX owner-only bits on Windows (no ACL wire-up), so
    // the same write reports 0o666 there; the guarantee holds on POSIX hosts.
    expect((await stat(preset.path)).mode & 0o777).toBe(process.platform === 'win32' ? 0o666 : 0o600)
